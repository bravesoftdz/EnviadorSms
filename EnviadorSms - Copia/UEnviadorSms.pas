...
begin
  FEnviaSms.dbPrincipal.Connected := False;
...
begin
  ...
  try
    FEnviaSms.dbPrincipal.Connected := True;
...
begin
  Numero := 0;
  while Numero <=49 do begin
    FEnviaSms.Sms2.ApagarSMS(IntToStr(Numero));
    //'AT+CMGD='+
    Numero := Numero + 1;
  end;
end;
...
begin
  ...
  Try
    ...
    FEnviaSms.Sms2.ListarMensagens(fltNaoLidas,'Sms.txt');
    ShowMessage(FEnviaSms.Sms2.UltimoComando);
    ShowMessage(FEnviaSms.Sms2.UltimaResposta);
    LimpandoChip;
  except on E: Exception do
...
begin
  if FEnviaSms.btAtivarDesativar.Caption = 'Desativar' then begin
...
procedure TFEnviaSms.BitBtn2Click(Sender: TObject);
begin
  FEnviaSms.Sms2.Device.Porta := 'COM6';
  FEnviaSms.Sms2.Device.Baud  := 115200;
  FEnviaSms.Sms2.Ativar;
  FEnviaSms.Sms2.EnviarComando('AT+CMGL');
  ShowMessage(FEnviaSms.Sms2.UltimaResposta);
  FEnviaSms.Sms2.Desativar;
end;
...
begin
  if edNumeroCel.Text <> '' then begin
    VerificarRemoto;
...
begin
  ...
  if (VerificaConexao)  then begin
    ...
    if (StrToInt(lbEnviados.Caption) MOD 40  = 0) and (Trim(lbEnviados.Caption)<>'0') then begin
...
begin
  if FileExists('SmsRecebidos.txt') then begin
...
begin
  if ckVerificaRecebidos.Checked then begin
    VerificaSmsRecebido;
